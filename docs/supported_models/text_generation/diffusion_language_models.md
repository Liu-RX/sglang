...
# Supported verification modes are 1, 2, and "spiffy".
# Prefix/suffix matching and verify_batch_size=4 are not enabled here.
...
To enable speculative decoding, put the options above in a YAML file and pass it
with `--dllm-algorithm-config`. For example:
...
python3 -m sglang.launch_server \
  ...
  --trust-remote-code \
  --dllm-algorithm LowConfidence \
  --dllm-algorithm-config examples/dllm_low_confidence_spec_spiffy.yaml \
